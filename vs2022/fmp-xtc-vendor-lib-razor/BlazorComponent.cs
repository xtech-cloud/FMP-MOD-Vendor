...
namespace XTC.FMP.MOD.Vendor.LIB.Razor
{
    ...
    {
        private BlazorFacade? getFacade()
        {
            if (null == facade_)
            {
                if (null == framework_)
                ...
                var entry = framework_.getUserData("XTC.FMP.MOD.Vendor.LIB.MVCS.Entry") as Entry;
                if (null == entry)
                {
                ...
                facade_ = entry?.getDynamicBlazorFacade("default");
                if (null == facade_)
                {
                    logger_?.Error("facade_ is null");
                    return null;
                }
                facade_?.setUiBridge(new BlazorUiBridge(this));
            }
            return facade_;
        }
